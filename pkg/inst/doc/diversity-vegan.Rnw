...
\author{Jari Oksanen}
\date{Based on $ $Id$ $, processed with vegan
  \Sexpr{packageDescription("vegan", field="Version")} in
  \Sexpr{R.version.string} on \today}
\begin{document}
...
graphically (Fig. \ref{fig:renyi}):
<<a>>=
matplot(t(R), type="l", axes=FALSE, xlab="Renyi parameter", ylab="Diversity")
axis(1, at=1:11, labels=colnames(R))
axis(2)
legend("topright", legend=k, lty=1:5, col=1:5)
box()
@
\begin{SCfigure}
<<fig=true,echo=false>>=
<<a>>
@
\caption{Rényi diversities in five randomly selected plots.}
\label{fig:renyi}
\end{SCfigure}
...
\begin{equation}
\label{eq:rarevar}
s^2 = p_i (1-p_i) + 2 \sum_{i=1}^S \sum_{j>i} \left[ {N- x_i - x_j \choose n} / {N
...
The number of stems per hectar varies in our
data set:
<<>>=
...
to these inividuals.  Kindt's exact accumulator resembels rarefaction:
\begin{equation}
\label{eq:kindt}
...
\begin{equation}
s^2 = \sum_{i=1}^{S_o} q_i (1-q_i) + 2 \sum \sum Z_p
\end{equation}
which needs a much longer explanation\dots
...
species of abundant and rare species, with an arbitary upper limit of
...
plot(smo[,j], BCI[,j], main="Ceiba pentandra", xlab="Probability ofoccurrence",
ylab="Occurrence")
@
\begin{SCfigure}
<<fig=true,echo=false>>=
