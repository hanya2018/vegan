...
%\VignetteIndexEntry{introduction to vegan}
...
and a wrapper for nonmetric multidimensional scaling
...
automatically knows how to access the scorees.
...
package.  Function \texttt{metaMDS} is a wrapper to perform nonmetric
...
species scores to the configuration as weighted avereges (function
\texttt{wascores}):
<<>>=
ord <- metaMDS(dune)
ord
...
dynamic three-dimensional graphics that can be spinned around or
zoomed into with your mouse.  Function \texttt{ordiplot3d} (requires
package \texttt{scatterplot3d}) displays simple three-dimensional
scatterplots.
...
constraints.  The test mimicks standard analysis of variance function
